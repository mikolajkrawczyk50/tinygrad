...
class MGLAllocator(LRUAllocator['MGLDevice']):
  def __init__(self, dev:'MGLDevice'): super().__init__(dev, supports_transfer=False)

  def _alloc(self, size:int, options:BufferSpec) -> moderngl.Buffer:
    # OpenGL buffers have to be 4-byte aligned; add 256B padding to guard against speculative GPU loads past buffer end
    with self.dev.ctx: return self.dev.ctx.buffer(reserve=round_up(size + 256, 4))

  def _free(self, opaque:moderngl.Buffer, options:BufferSpec):
    with self.dev.ctx: opaque.release()

  def _offset(self, buf:MGLBuf, size:int, offset:int) -> tuple[moderngl.Buffer, int]:
  ...
  def _copyin(self, dest:MGLBuf, src:memoryview):
    ...
    with self.dev.ctx:
      if src.nbytes % 4:
        ...
        buf.write(padded_src, offset=offset)
      else:
        buf.write(src, offset=offset)
